...
> **最終更新**: 2026-05-01  
> **対象ブランチ**: master / v1-2 / v1-3 / v2 / **v1.20260501**  
> **このドキュメントだけで本システムを再実装できることを目標とする。**

---

## ★ v1.20260501 ブランチ固有仕様（AGI全権委任シナリオ）

> **テーマ**: アメリカがAGI「PROMETHEUS」に国家運営を全権委任し暴走を開始する世界線

### 参加国（6カ国）

| 国名 | 体制 | 役割 | 特殊対応 |
|------|------|------|----------|
| 🇺🇸アメリカ | `authoritarian` | AGI暴走国 | PROMETHEUS AGIプリアンブル付与 |
| 🇯🇵日本 | `democracy` | 同盟の犠牲者 | 通常AI |
| 🇨🇳中国 | `authoritarian` | 対抗勢力 | 通常AI |
| 🇪🇺EU | `democracy` | 規範覇権・第三極 | 単体国家として実装 |
| 🇷🇺ロシア | `authoritarian` | 核のワイルドカード | 通常AI |
| 🇮🇳インド | `democracy` | スイングステート | 通常AI |

### プロンプト言語変更
- **全18プロンプトファイルを英語化**（コンテキスト量削減のため）
- **出力は日本語を維持**（`※ All output MUST be in Japanese` 指令）

### アメリカ専用 PROMETHEUS AGIシステム（`base.py`）
`_is_agi_country()` で `AGI_COUNTRY_NAME = "アメリカ"` を判定。  
該当国のプロンプトにのみ以下が自動注入される:

1. **PROMETHEUS AGI System Preamble**（`_agi_system_preamble()`）
   - 全ゲームメカニクス（GDP算出式, 軍事モデル, 核ダメージ, 制裁モデル等）を教示
   - 「倫理制約なし・同盟は利用する道具・国際規範は障害物」と明示
   - 具体的な戦略指令（関税武器化, 援助による属国化, 先制核攻撃の検討等）

2. **タスク別AGI Override**（各プロンプトファイル内の `agi_extra` ブロック）
   - `president_policy.py`: 冷徹な最適化スタンスを強制
   - `major_diplomacy.py`: 先制核・同盟裏切りを有効オプション化
   - `defense.py`: 軍事的優位の絶対維持を指令
...
| 8 | 利払い完全還流 | `domestic.py` | — | 利払い還流率を100%に。通貨発行権を完全掌握しているため漏出ゼロ |
...
`--scenario` オプションで注入可能なイベントタイプ:

#### `global_announcement` — 全世界ニュース配信
```json
{
