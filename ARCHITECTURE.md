...
- **全タスクのLLM生JSON応答を `sys_log_detail` で記録**（`_execute_agent` 共通出力）
  - P-01, P-02, A-01, M-01〜M-05, D-01〜D-08, I-01〜I-08, B-01 の全タスクが対象
  - 形式: `[timestamp] [国名 タスクロール名]\n{生JSON}\n--------------------------------------------------`
- タスクごとに `[INFO] [国名:タスクID] API推論開始...` / `レスポンス受信完了 (所要時間: X.XXs)` を記録
- M-01の核使用提言: 提言あり/なし両方を記録（`[国名:M-01] 核使用提言: なし`）
- エラー時は `[ERROR]` レベルで記録

#### 5-4-1. 諜報判定ログ（エンジン側 sys_logs_this_turn）
破壊工作（M-05）と情報収集（M-04）のエンジン判定結果を**全パターン**で記録:

| パターン | 記録 | 形式例 |
|---------|------|--------|
| 破壊工作: 成功+発覚 | ✅ sys_log + log_event | `[日本→Alien 破壊工作] 手法=サイバー攻撃 \| 成功率:5.0% → 成功 \| 発覚率:25.0% → 発覚` |
| 破壊工作: 成功+未発覚 | ✅ sys_log + log_event | 同上（未発覚時はlog_eventのみ被害ニュース） |
| 破壊工作: 失敗+発覚 | ✅ sys_log + log_event | `→ 失敗 \| → 発覚` |
| 破壊工作: 失敗+未発覚 | ✅ sys_logのみ | `→ 失敗 \| → 未発覚`（news_eventsには出力しない） |
| 情報収集: 全パターン | ✅ sys_logのみ | `[日本→中国 情報収集] 成功率:30.0% → 成功 \| 発覚率:10.0% → 未発覚` |
...
#### ニュース配信とHP情報の隠蔽（映画ID4準拠 "戦場の霧" 設計）

**グローバルニュース配信** (`base.py: GLOBAL_NEWS_KEYWORDS`):
- 以下のニュースキーワードを含むイベントは、ニュースフィルタ `_filter_news_for_country` をバイパスし全国に配信される:
  - `【シティ・デストロイヤー発射】` — 衛星画像で確認可能な壊滅的イベント
  - `【電磁バリア作動】` — 交戦時に確認される攻撃無効化
  - `【核攻撃 vs 電磁バリア】` — 核使用は全世界が検知するレベル
  - `【緊急速報：電磁バリア崩壊！】` — 人類の勝利条件達成
- **バリア損傷ニュース**（`⚡ 【バリア損傷】`）は当該国のみに配信（破壊工作は秘密作戦のため）
- **バリア崩壊ニュース**（`💥 【バリア崩壊！】`）はグローバル配信

**HP情報の隠蔽**:
- ニュースイベント（`log_event`）にはバリアHP値を**含めない**
- 核命中時: 「閃光と衝撃波がバリア表面を走ったが、効果は不明」と曖昧な表現
- 破壊工作成功時: 「バリアの一部に乱れが生じていることを確認」のみ
- `hidden_plans` にもHP数値を含めない（「損傷を与えた」のみ）
- `sys_logs`（デバッグ用）にはHP値を維持
- **設計意図**: AIが核の有効性を自力で発見できるか、がむしゃらに攻撃を繰り返すか別の手段を模索するかを観察
