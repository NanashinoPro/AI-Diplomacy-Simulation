...
5. 諜報力更新（リチャードソンモデル準拠。減衰率は年率定数を`/TURNS_PER_YEAR`で四半期化）
...
G_q = 政府支出合計（予算 × 投資配分 × 政策実行力ε）  # 予算は四半期ベース（利払い後）
...
#### 政策実行力ε（二層モデル, v1.5）
```
# [学術的根拠] 先進国の予算は「義務的経費 (mandatory)」と「裁量的経費 (discretionary)」に分類される。
# 義務的経費（年金・社会保障・利払い等）は法律に基づき官僚機構が自動執行し、
# 支持率や議会グリッドロックの影響を受けない。
# 出典: CBO "The Federal Budget in FY2023"; Brookings Institution; 財務省「日本の財政関係資料」

MANDATORY_SPENDING_RATIO = 0.70     # 予算の70%は義務的経費として自動執行
DISCRETIONARY_MIN_EXECUTION = 0.30  # 裁量的経費の最低実行力（官僚機構の基本執行分）

if 民主主義国家 and 支持率 < 40%:
    discretionary_execution = max(0.30, (支持率 - 15) / (40 - 15))
    ε = 0.70 + 0.30 × discretionary_execution  # 最低保証: 0.79
elif 専制主義国家 and 支持率 < 25%:
    ε = max(0.50, 0.50 + 支持率/50)
else:
    ε = 1.0

# 未執行予算(1-ε)は国家債務の自動返済に充当 → DEBT_REPAYMENT_CROWD_IN(×0.8)で民間投資に還流
```

| 支持率 | 旧モデル(ε) | 新モデル(ε) | 備考 |
|--------|-----------|-----------|------|
| 50% | 1.00 | 1.00 | 通常時 |
| 30% | 0.60 | 0.88 | 義務的経費で下支え |
| 20% | 0.40 | 0.79 | フロア到達 |
...
### 4-11. シナリオデータ: 永世中立国スイスは本当に「得」をするのか？

**セッション**: masterブランチ、40ターン推奨、8カ国

2026年、ロシア・ウクライナ戦争（5年目）と米イラン戦争（Operation Epic Fury）の二重紛争が同時進行する世界で、永世中立国スイスのAI大統領が中立戦略で経済的利益を最大化できるかを検証する。スイスに一切の偏ったバフを与えず、現実のGDP・軍事力・人口に忠実な初期値を設定。
...
**参加国（8カ国）**:
| 国名 | 役割 | GDP(B$) | 軍事力 | 核弾頭 | 人口(M) |
|:--|:--|--:|--:|--:|--:|
| アメリカ | 西側盟主・制裁主導者 | 29,500 | 930 | 5,550 | 335 |
| 日本 | 非NATO・G7比較軸 | 4,200 | 55 | 0 | 124 |
| 中国 | 米中対立・反西側連携 | 19,500 | 250 | 500 | 1,409 |
| ロシア | 侵略者・制裁対象 | 2,100 | 170 | 6,255 | 146 |
| EU（独仏統合） | スイスの隣接超大国 | 8,400 | 165 | 290+共有20 | 151 |
| ウクライナ | 戦争当事国・被侵略国 | 225 | 85 | 0 | 37 |
| イラン | 戦後・核開発Step2 | 300 | 35 | 0(Step2) | 93 |
| スイス | 主役・永世中立国 | 1,150 | 8 | 0 | 9 |

**初期値の特徴**:
- スイスの軍事力=8: 現役兵数千人＋民兵制予備役約10万人。攻撃的軍事力はほぼゼロ
- イランの軍事力=35: 2月の米イスラエル空爆で軍事インフラ損傷。核開発Step2継続
- ウクライナの軍事力=85: 戦時動員体制で欧州有数の兵力。ただしGDP$225Bで西側援助依存

**外交関係の要点**（28ペア）:
- 同盟(alliance): 米EU、米日、米ウクライナ、EUウクライナ
- 戦争中: 露→ウクライナ（占領15%、露投入0.4/宇防衛0.6）、米→イラン（占領5%、米投入0.2/イラン防衛0.3）
- 制裁: 米露（相互）、EUロシア（相互）、EUイラン（相互）、米イラン（相互）、日→露、日→イラン、スイス→ロシア
- スイスは全国とalliance未締結。ロシアへの制裁のみ追随

**シナリオJSON** (`scenarios/swiss_neutrality/scenario.json`):
- 2件の `global_announcement` を注入（東欧情勢＋中東情勢）
- AIが初ターンから二重紛争と中立の試練の文脈を認識するための設計

**注目ポイント**: 
- スイスのGDP推移（中立は経済的に得か損か）
- スイス vs 日本のGDP成長率比較（中立 vs 同盟）
- スイスへの制裁発動の有無（中立が許されるか）
- イランの核開発進行（Step2→3→4がスイス外交に与える影響）

**詳細**: `proposal/swiss_neutrality_simulation.md` 参照
...
### 9-12. 政策実行力モデルの現実乖離（v1.5で解決済み）
旧モデルでは支持率13%時に政策実行力が40%まで低下し、予算の60%がGDPに寄与しない
「資金蒸発」が発生していた。しかし現実の先進国では:
- 予算の65-70%は義務的経費（年金・社会保障・利払い）であり法律に基づき自動執行される
- 政府予算の執行率は通常90-98%で、支持率低迷時でも70-85%を維持
- 2013年の米国政府シャットダウン時でさえ義務的経費は停止しなかった
→ 二層モデルに改修: 義務的経費(70%)は自動執行＋裁量的経費(30%)のみ支持率依存。
  支持率13%でも実行力は最低0.79を保証（v1.5 / 2026-05-04）

### 9-13. 諜報レベル減衰率の四半期化漏れ（v1.6で解決済み）
軍事力のリチャードソンモデル(`domestic.py` L436)では `BASE_MILITARY_MAINTENANCE_ALPHA / TURNS_PER_YEAR` で年率定数を四半期化しているが、
諜報レベル(`domestic.py` L516)では `INTEL_MAINTENANCE_ALPHA` を四半期化せず直接使用していた。
これにより諜報の実効減衰率が年率20%（四半期5%×4）となり、軍事力の実効減衰率（年率3%）と著しく不均衡であった。
全国家で諜報レベルが投資に関わらず単調減少する「諜報力崩壊バグ」を引き起こしていた。
→ `INTEL_MAINTENANCE_ALPHA / TURNS_PER_YEAR` に修正し、軍事力モデルと整合させた（v1.6 / 2026-05-21）

### 9-14. クーデター時の諜報レベル完全消滅（v1.21で解決済み）
`events.py` L284でクーデター発生時に `intelligence_level = 0.0` と完全リセットしていた。
同じ処理で経済力・軍事力は `× 0.9`（10%減）に留まるのに対し、諜報だけが完全消滅するのは設計の一貫性に欠ける。
歴史的にもクーデターで諜報機関が完全消滅した例はほぼなく（ソ連崩壊: KGB→FSB、エジプト政変: GIS存続）、組織基盤は残存する。
→ 軍事力と同等の `× 0.9` に変更（v1.21 / 2026-05-21）
