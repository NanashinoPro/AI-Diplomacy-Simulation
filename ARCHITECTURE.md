...
*   **国力・経済指標:** 経済力 (GDPベース $Y$)、政府予算 ($G$)、軍事力 ($M$)、国家累積債務 (National Debt)、税率 (Tax Rate)。
*   **社会的指標:** 政府支持率 ($\alpha$, 0%〜100%)、潜在的反乱リスク (Rebellion Risk)。
...
計算された $\epsilon$ が、政府支出の各項目 ($G_{econ}, G_{mil}, G_{wel}$) に乗算され、投資の実行効果が減衰する。
...
4.  **国家債務と経済ペナルティ**:
    ...
    *   国家累積債務がGDPの100%を超過した場合、インフレーションや通貨信認低下によるデバフが当期の暫定GDPに乗算される。
    *   $Penalty = 1.0 - \min(0.05,\ (\frac{Debt}{GDP} - 1.0) \times 0.02)$ （最大 $5\%$ の押し下げ）
5.  **福祉ボーナスによる支持率還元**:
...
#### 純輸出 (NX) の移転と貿易赤字ペナルティ
ISバランス（$(S - I) + (T - G)$）の差分から、ベース取引量の一部が赤字国から黒字国へと移転する。赤字国（輸入超過国）の流出分は純輸出(NX)マイナスとして直ちに「国家累積債務 (National Debt)」に加算される。
*   **支持率への影響**：赤字国は物価安定や消費者余剰（安価な輸入品）の観点から短期的な支持率ボーナス（$+1.0\%$）を受けるが、黒字国は $+0.5\%$ となる。
*   **産業空洞化ペナルティ**：貿易赤字カウンタが **4ターン以上**（`trade_deficit_counter > 3`）に達した場合、長期的な失業増大を反映し、赤字国の支持率ボーナスから以下のペナルティが差し引かれる。
    *   $Penalty = \min(5.0,\ (DeficitCounter - 3) \times 1.5)$ （最大 $-5.0\%$）
...
戦力差（Power Ratio: $PR = (TotalPower_{Attacker} - TotalPower_{Target}) / TotalPower_{Target}$）を用いてダイスロールの成功確率・発覚確率が動的に変動する。
工作の成否と被害量は、単純な確率だけでなく、LLMがプロンプトで記述した文字列の自然言語的特徴に基づく。

#### 情報収集 (Gather Intel)
相手国の裏ステータス（`hidden_plans`）を探る工作。
*   **成功確率**: $\min(0.60, \max(0.15, 0.30 + PR \times 0.1))$
*   **発覚確率**: $\min(0.30, \max(0.05, 0.10 - PR \times 0.1))$

#### 破壊工作 (Sabotage)
インフラ破壊や世論工作。
*   **成功確率**: $\min(0.35, \max(0.05, 0.15 + PR \times 0.1))$
*   **発覚確率**: $\min(0.50, \max(0.10, 0.25 - PR \times 0.2))$
...
*   4年（16ターン）ごとの大統領選挙を迎えた際、落選確率 $= (100 - Approval) / 100$ で合否判定が行われる。敗北時の新政権の支持率は政権交代への期待として $100.0 - Approval$ に反転してセットされる。
