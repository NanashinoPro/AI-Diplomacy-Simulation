...
### 2.3. 人口動態モデル (Demographic Transition Model)
各国の人口増減は、出生率と死亡率という2つのマクロ関数の差分（自然増減）によってシミュレートされる。
1. **少子化の罠（出生率の低下）**：1人当たりGDPと教育水準 ($H$) が高いほど、多産多死から少産少死へと移行し出生率が急速に低下する。これを緩和するには、政府内政で福祉（$G_{wel}$）に予算を割き、子育て支援等を行う必要がある。
2. **死亡率**：基準死亡率に加え、1人当たりGDPが低すぎる場合は「貧困・飢餓」による死亡補正が加わる。また、災害ダメージ発生時にも一時的に死亡率が上昇する。
人口増加率がGDP成長率を下回ったり人口減少が続くと、労働力不足により 1人当たりGDP が低下し、次項§2.4の暴動リスクを誘発する最大の要因となる。
...
### 2.6. 反乱・選挙と国家分裂モデリング (Rebellion, Election & State Fragmentation)
...
*   **離脱リソースと「事実上の国家転覆」**:
    *   不満（BaseInstability）が高いほど、新国家が奪取する国土・経済等のリソース割合（$Ratio$）が増加する。
...
### 2.10. 国家崩壊とデータのクリーンアップ (Country Collapse & Cleanup)
