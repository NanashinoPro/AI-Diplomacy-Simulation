...
`join_ally_defense: true` + `defense_support_commitment: 投入率（0.01〜0.50）` を設定すると、同盟国が防衛側となっている既存の戦争に「防衛支援国」として参加できます。
- **target_countryには攻撃国（敵国）を指定**してください。直接宣戦布告（declare_war）とは異なります。
- 自国軍の一部が同盟国の防衛に合流し、防衛側の戦力が増強されます。投入分のみが損害を受けます。
