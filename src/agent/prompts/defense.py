...
- ⚠️ **後方防衛リスク（極めて重要）**: 投入した軍事力は前線に固定され、**残りの未投入分でしか他国からの奇襲・宣戦布告に対抗できない**。投入率0.7なら防衛に使えるのは全軍の30%のみ。高い投入率は「第三国に背後を突かれたら壊滅する」リスクと引き換えである。特に複数の潜在的敵国と国境を接する大国（例: 中国はインド・ロシアとも国境を接する）は、投入率0.5以上は極めて危険
- ⚠️ **動員速度制限**: 1ターンあたりの変動幅は**±10%**に制限されます（例: 現在0.20→次ターン最大0.30）。急激な動員拡大は兵站・訓練の制約上不可能です。段階的な変更を計画してください
