...
【Rules】0.0-1.0. Lowering strengthens information control and reduces covert operation exposure risk. However, causes immediate approval drop.

Following the policy, determine {country_name}'s press freedom level. You MUST respond in Japanese.
Output ONLY JSON (no code blocks, determine value yourself):
{{"target_press_freedom": ???, "reason": "reason (max 30 chars)"}}
"""
...
def build_deception_prompt(country_name, country_state: CountryState, world_state: WorldState, policy: PresidentPolicy, past_news=None) -> str:
    ...
    ctx = build_common_context(country_name, country_state, world_state, past_news, role_name="Information Officer (External Reporting)")
    return ctx + build_policy_section(policy) + f"""
...
【Rules】None = publish true values. Intentional deviation can mislead other countries' judgments,
but risks exposure by media. Set all fields to null if no deception.

You MUST respond in Japanese. Output ONLY JSON (set all to null if no deception):
...
def build_parliament_prompt(country_name, country_state: CountryState, world_state: WorldState, policy: PresidentPolicy, past_news=None) -> str:
    """I-08: Parliament Dissolution Decision (flash-lite) - Democracy only"""
...
【Rules】dissolve_parliament=true to dissolve parliament. Success rate = approval%. Failure causes regime change. Cost = GDP×0.01-0.02%.

You MUST respond in Japanese. Output ONLY JSON:
{{"dissolve_parliament": false, "reason": "reason (max 30 chars)"}}
"""
