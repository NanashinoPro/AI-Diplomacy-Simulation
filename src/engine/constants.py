...
# --- 海峡封鎖マップ ---
# キー: 海峡名
# 値: その海峡が封鎖されたときに遮断される「域外ソース」キー（__始まり）のリスト
STRAIT_BLOCKADE_MAP: dict = {
    "ホルムズ海峡": ["__中東"],
    # サウジアラビア・イランはシミュ内国家 → STRAIT_EXPORT_BLOCKED_COUNTRIES で別途制御
    "マラッカ海峡": ["__その他"],
}

# --- 海峡封鎖時に輸出停止になるシミュ内国家 ---
# キー: 海峡名 / 値: エネルギー輸出が停止される国名リスト
STRAIT_EXPORT_BLOCKED_COUNTRIES: dict = {
    "ホルムズ海峡": ["サウジアラビア", "イラン"],
}

# --- 海峡封鎖を宣言できる国（封鎖資格国）---
# [根拠]
#   イラン: ホルムズ海峡沿岸国。IRGC海軍が制海権を脅かす(2019等に複数回実施)
#   アメリカ: 第5艦隊（バーレーン拠点）が湾内に常駐。海軍封鎖が現実的
#   サウジアラビア: 自国輸出停止による実質的な供給封鎖（1973年OAPECの先例）
STRAIT_BLOCKADE_ELIGIBLE_COUNTRIES: dict = {
    "ホルムズ海峡": ["イラン", "アメリカ", "サウジアラビア"],
}

# --- 危機ステージ閾値（目標備蓄に対する割合）---
# [根拠] IEA90日備蓄義務 = 1.0ターンを基準に設定
# Warning: 目標の50%未満 → 節電・燃料費高騰が顕在化
# Critical: 目標の25%未満 → 工場停止・停電が現実的
ENERGY_WARNING_RATIO = 0.50
ENERGY_CRITICAL_RATIO = 0.25

# --- economy ペナルティ係数（/ターン）---
# [根拠]
#   1973年オイルショック(日本): 石油40%遮断 → 年間約-9% GDP (4T) → 約-2.25%/T
#   2022年欧州ガス危機: ガス60%遮断 → 年間-0.7% GDP (2.5T) → 約-0.28%/T
#   比例モデル: per 1% deficit ≈ 0.04%/T (線形); deficit≥50%で非線形加速(0.08%/T)
#   (Hamilton 1983, JPE; IMF WEO 2022)
ENERGY_ECONOMY_PENALTY_LINEAR = 0.04    # deficit × coeff [%/ターン] (deficit < 50%)
ENERGY_ECONOMY_PENALTY_NONLINEAR = 0.08 # deficit ≥ 50%時の加速係数

# --- approval_rating ペナルティ（/ターン）---
# [根拠]
#   1973年オイルショック: 田中角栄内閣 50%→20% (約2T) → -15pt/T (極端ケース)
#   2022年欧州: 各国首相の平均 10-15pt/年 ≈ 3-4pt/T (穏やかケース)
#   比例モデル: deficit_ratio × 15pt/T (最大-15pt at 100%遮断)
ENERGY_APPROVAL_PENALTY_COEFF = 15.0
