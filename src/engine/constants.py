...
# 動的信用スプレッドモデル（Harvard研究: 債務GDP比連動）
...
# --- 政治・実行力モデル定数（二層モデル: 義務的経費 + 裁量的経費）---
# [学術的根拠] 予算の約70%は法律に基づく義務的経費（年金・社会保障・利払い等）であり、
# 支持率や議会の攻防に関係なく官僚機構により自動執行される。
# 残り約30%が裁量的経費であり、こちらのみ支持率による実行力低下の影響を受ける。
MANDATORY_SPENDING_RATIO = 0.70       # 義務的経費の予算比率（年金・社会保障等、支持率非依存）
DISCRETIONARY_MIN_EXECUTION = 0.30    # 裁量的経費の最低執行率（官僚機構による基本執行分）
DEMOCRACY_MIN_EXECUTION_POWER = 0.79  # 民主主義における政策実行力の最低保証値（0.70 + 0.30 * 0.30）
