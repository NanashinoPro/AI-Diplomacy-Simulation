...
TAX_APPROVAL_PENALTY_MULTIPLIER = 100.0 # 増税1%につき支持率が1%低下する係数（v2: 200→100に半減。安倍内閣消費増税データに基づく）
...
# --- 政治・実行力モデル定数 ---
# [学術的根拠] 先進国の予算は「義務的経費（mandatory spending）」と「裁量的経費（discretionary spending）」に
# 二分される。義務的経費（年金・社会保障・利払い等）は法律に基づき官僚機構が自動執行するため、
# 支持率や議会グリッドロックの影響を受けない。裁量的経費のみが政治的実行力に左右される。
#   - 米国連邦予算: 義務的経費 ≈ 65-70% (CBO 2024)
#   - 日本一般会計: 義務的経費（社会保障+国債費） ≈ 55-60% (財務省)
#   - EU諸国平均: ≈ 60-70% (Eurostat)
# 出典: CBO "The Federal Budget in Fiscal Year 2023"; 財務省「日本の財政関係資料」
MANDATORY_SPENDING_RATIO = 0.70      # 義務的経費比率（予算の70%は自動執行）
DISCRETIONARY_MIN_EXECUTION = 0.30   # 裁量的経費の最低実行力（民主主義国家、官僚機構の基本執行分）
DEMOCRACY_MIN_EXECUTION_POWER = 0.79 # [旧フロア値。新モデルでは MANDATORY + (1-MANDATORY)*DISCRETIONARY_MIN = 0.70+0.30*0.30 = 0.79]
...
# --- シティ・デストロイヤー（Alien超兵器）定数 ---
# [設計根拠] 戦略核5発(GDP-30%)の約半分のダメージ。都市単体壊滅レベル。
# チャージ1ターン（1四半期）後に発射可能。Alien AIが対象を選択する。
CITY_DESTROYER_ECON_DAMAGE = 0.15      # 経済ダメージ率（GDP × 15%）
