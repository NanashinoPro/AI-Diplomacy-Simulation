...
from models import (
    AgentAction, MilitaryDeploymentState,
...
class MilitaryMixin:
    ...
    def _process_military_deployments(self, actions: Dict[str, AgentAction]):
        """全国のAI出力から軍事配備状態を更新（軍事費ベース・バリデーション付き）"""
        for country_name, action in actions.items():
            country = self.state.countries.get(country_name)
            if not country:
                continue
            
            # 配備可能な軍事予算上限 = 軍事力ポイント（≒十億ドル）
            max_budget = country.military
            ...
            # deployments のバリデーション
            valid_deployments = []
            budget_by_type = {"army": 0.0, "navy": 0.0, "air": 0.0}
            ...
            for d in action.deployments:
                ...
                if d_type == "navy" and not country.has_coastline:
                    self.sys_logs_this_turn.append(
                        f"[{country_name} 配備] 内陸国のため海軍配備をスキップ"
                    )
                    continue
                ...
                budget_by_type[d_type] = budget_by_type.get(d_type, 0.0) + d.budget_amount
            
            # 予算上限チェック: 合計が軍事力を超えたら按分スケールダウン
            total_budget = sum(d.budget_amount for d in valid_deployments)
            if total_budget > max_budget and total_budget > 0:
                scale = max_budget / total_budget
                for d in valid_deployments:
                    d.budget_amount *= scale
                # スケールダウン後の集計を更新
                budget_by_type = {"army": 0.0, "navy": 0.0, "air": 0.0}
                for d in valid_deployments:
                    d_type = d.type.value if hasattr(d.type, 'value') else str(d.type)
                    budget_by_type[d_type] = budget_by_type.get(d_type, 0.0) + d.budget_amount
                self.sys_logs_this_turn.append(
                    f"[{country_name} 配備] 予算超過のため按分スケールダウン "
                    f"(要求: ${total_budget:.1f}B → 上限: ${max_budget:.1f}B, 比率: {scale:.2f})"
                )
            ...
            self.sys_logs_this_turn.append(
                f"[{country_name} 配備完了] "
                f"陸軍: ${budget_by_type.get('army', 0):.1f}B, "
                f"海軍: ${budget_by_type.get('navy', 0):.1f}B, "
                f"空軍: ${budget_by_type.get('air', 0):.1f}B "
                f"(合計: ${total_budget:.1f}B / 上限: ${max_budget:.1f}B, "
                ...
                for d in attacker.military_deployment.deployments:
                    ...
                    if d_type == "air":
                        ...
                        if mission == "strategic_bombing":
                            econ_damage = target.economy * 0.003 * d.budget_amount
                            target.economy = max(1.0, target.economy - econ_damage)
                            self.sys_logs_this_turn.append(
                                f"[戦略爆撃] {atk_name} → {tgt_name}: 経済ダメージ {econ_damage:.1f} (${d.budget_amount:.1f}B投入)"
                            )
                    elif d_type == "navy":
                        mission = d.naval_mission.value if d.naval_mission and hasattr(d.naval_mission, 'value') else ''
                        if mission == "blockade":
                            blockade_damage = target.economy * 0.0015 * d.budget_amount
                            target.economy = max(1.0, target.economy - blockade_damage)
                            self.sys_logs_this_turn.append(
                                f"[海上封鎖] {atk_name} → {tgt_name}: 貿易遮断ダメージ {blockade_damage:.1f} (${d.budget_amount:.1f}B投入)"
                            )
