        ...
        if len(world_state.countries) <= 1:
            ...
            for p_name, p_state in world_state.countries.items():
                ...
                other_info += (
                    ...
                    f"関係={rel.value}{war_info}{suzerain_info}\n"
                )
...
A. 内政的解決（痛みを伴う緊縮財政と適正税率への回帰）:
   ...
   - 税率(`tax_rate`)の調整: 極端な減税に偏り国債が膨張すると、国家信認の低下により最終的に経済成長率が致命的なデバフを受けます。支持率が一時低下しても、持続可能な税率（例: 25〜35%程度）を維持・回復させ、借金を減らす（増税や緊縮）ことが長期的な国家生存には不可欠です。
B. 外交的解決（他国への強硬手段）:
...
【人口動態と福祉投資（invest_welfare）の必須ルール】
あなたの国は「人口転換理論」に基づいており、1人当たりGDPおよび教育水準が上がると「少子化の罠」により出生率が低下し、労働力（人口）が減少してマクロ経済の成長が鈍化します。
これを回避・緩和するには、`invest_welfare`（福祉）への一定の投資が不可欠です。福祉は支持率の維持だけでなく、少子化に対抗し労働力を維持するための重要な国策です。
