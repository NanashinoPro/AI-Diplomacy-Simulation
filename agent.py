    ...
    def _build_prompt(self, country_name: str, country_state: CountryState, world_state: WorldState, past_news: List[str] = None) -> str:
        ...
        if country_state.stat_history:
             ...
             for s in country_state.stat_history:
                  my_info += f" T{s['turn']}: 経済力 {s['economy']}, 軍事力 {s['military']}, 諜報力 {s['intelligence_level']}, 支持率 {s['approval_rating']}%\n"
...
A. 内政的解決（痛みを伴う緊縮財政）:
   ...
   - 税率(`tax_rate`)の調整: 前期より増税すると税収(T)が増え赤字を減らせますが、消費が減り支持率が即時に低下します。逆に減税（税率の引き下げ）を行うと、民間消費が活性化してGDP成長に強力なボーナスがかかり、即座に支持率も上昇します。経済を刺激したい場合や支持率を稼ぎたい場合に有効です。
B. 外交的解決（他国への強硬手段）:
...
教育・科学への投資は、短期的には大きな効果が見えにくいですが、中長期的に「教育・人的資本レベル」を蓄積させます。
- 人的資本（知識・技能）が蓄積されると、同じ労働や資本の投入であっても、より大きなGDP成長（産出弾力性バフ）を生み出します。
- 国家を長期的に世界一の経済大国へと導くための最重要投資の一つです。
...
以下のJSONスキーマに従って出力してください。必ずJSONオブジェクトのみを出力し、それ以外のテキストは含めないでください。
{
  ...
  "domestic_policy": {
    "tax_rate": 0.10から0.70の数値（増税で予算増加・支持率低下、減税で経済成長・支持率上昇ボーナス。⚠️1ターンあたりの変動は±10%が上限。例: 現在30%なら20%〜40%の範囲内で指定すること。急激な増税は社会不安と支持率の壊滅的低下を招く）,
    ...
    "invest_intelligence": 0.0から1.0の数値（諜報・技術開発への投資）,
    "invest_education_science": 0.0から1.0の数値（教育・科学技術への投資。人的資本を蓄積し、長期的な経済成長の土台となる）
  },
  "diplomatic_policies": [
    {
