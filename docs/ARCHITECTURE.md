...
#### ロジスティック方程式と環境収容力 ($K$)
*   人口増加率はマルサス的（無限）な成長ではなく、国土面積に基づく環境収容力（$K = Area \times 150.0 \div 1,000,000$）に制約される**ロジスティック成長モデル** ($ \frac{dN}{dt} = r_q N(1 - \frac{N}{K}) $) に従う。ここで $r_q = \frac{r_{annual}}{TURNS\_PER\_YEAR}$ であり、出生率・死亡率は年率で定義された値を四半期ベースに変換して適用する（他のマクロ経済モデルと整合）。人口（百万人単位）が $K$（百万人単位）に近づくほど自然増加率はゼロに収束する。
...
2.  **感情分析**: 生成された記事テキストを `GeminiSentimentAnalyzer.analyze()` (`gemini-2.5-flash-lite`) に渡し、-1.0〜+1.0の感情スコアを算出。スコアの2倍値（-5.0〜+5.0にクランプ）が支持率補正値として使用される。
...
## 4. Strategic Map Visualization & Tactical Deployment System (戦略マップ可視化・戦術配備システム)

### 4.1. Overview (概要)
v2ブランチで導入された、HoI4スタイルの地政学的マップ可視化とAoEスタイルの戦術軍事配備システム。各ターン終了時に自動的にダッシュボード地図（PNG）を出力し、領土・配備・戦況を可視化する。

### 4.2. Map Rendering Engine (`src/map/`)

#### 4.2.1. Architecture
```
src/map/
├── __init__.py
├── styles.py          # ダークテーマ定義（GitHub Dark Base: #0d1117）
├── layers.py          # 領土描画（GeoJSON → geopandas → matplotlib）
├── military_units.py  # ユニットマーカー描画（Shapely座標算出）
└── renderer.py        # 3段レイアウト統合レンダラー
```

#### 4.2.2. 3段ダッシュボードレイアウト
1. **ヘッダー**: ターン番号、四半期/年号、タイトル、アクティブ戦争数
2. **地図**: 参加国は国別カラー、非参加国はチャコールグレー、海洋はダークネイビー
3. **フッター**: 国家一覧テーブル（NAME/GDP/MILITARY/APPROVAL/POP/GOV/STATUS）

#### 4.2.3. ユニット座標算出アルゴリズム
Shapelyの `nearest_points` と `boundary.intersection` を活用した6ステップの座標算出：
1. 自国ポリゴンの重心を基点として取得
2. 対象国の重心との方位角を計算
3. 自国ポリゴンの境界線から最寄点を取得
4. 陸軍→対象国の最寄境界線上、海軍→沿岸線から海面方向にオフセット、空軍→国内上空
5. 複数ユニットの重複回避（方位角に基づくスプレッド）
6. bbox外チェックとフォールバック

### 4.3. Free Deployment System (完全自由配備システム)

#### 4.3.1. 概要
防衛大臣AIが毎ターン、`force_allocation`（陸海空の兵科比率）と`deployments`（各方面への配備命令）を自律的に決定する。大統領AIが最終承認する。

#### 4.3.2. 兵科と任務

| 兵科 | ユニット | 任務 | 効果 |
|------|---------|------|------|
| 陸軍 | 師団 | offensive | 攻+20%/守-10% |
| 陸軍 | 師団 | defensive | 守+30%/攻-20% |
| 陸軍 | 師団 | intimidation | 戦闘効果なし、緊張度↑ |
| 陸軍 | 要塞 | light | 防御+25% |
| 陸軍 | 要塞 | heavy | 防御+50%（コスト高） |
| 海軍 | 艦隊 | patrol | 通商護衛 |
| 海軍 | 艦隊 | show_of_force | 砲艦外交（緊張度大幅↑） |
| 海軍 | 艦隊 | blockade | 海上封鎖（⚠️戦時のみ、経済ダメージ） |
| 海軍 | 艦隊 | naval_engagement | 艦隊決戦（⚠️戦時のみ） |
| 海軍 | 艦隊 | amphibious_support | 上陸支援（⚠️戦時のみ、攻+20%） |
| 海軍 | 艦隊 | shore_bombardment | 艦砲射撃（⚠️戦時のみ、敵-15%） |
| 空軍 | 飛行隊 | air_superiority | 制空権確保（攻+5%） |
| 空軍 | 飛行隊 | ground_support | 地上支援（⚠️戦時のみ、攻+15%） |
| 空軍 | 飛行隊 | strategic_bombing | 戦略爆撃（⚠️戦時のみ、敵経済ダメージ） |
| 空軍 | 飛行隊 | recon_flight | 偵察飛行（緊張度微↑） |

#### 4.3.3. ユニット上限の計算
- **陸軍師団数**: `(military × army_ratio) / (GDP_per_capita × 3.4) × 1000 / 10000`
- **海軍艦隊数**: `(military × navy_ratio) / 50`（海岸線がない国は0）
- **空軍飛行隊数**: `(military × air_ratio) / 30`

### 4.4. Tension Mechanics (緊張度メカニクス)

#### 4.4.1. 学術的根拠
- **Mueller (1970, 1973)**: Rally 'round the flag effect — 外部脅威に直面した際の国民の一時的結束
- **Schultz (2001)**: Audience Costs in Democratic States — 民主主義国家における威嚇のコスト
- **Fearon (1994)**: Domestic Political Audiences and the Escalation of International Disputes — 国内聴衆コスト理論

#### 4.4.2. 緊張度スコアの算出
各国の配備命令から、対象国ごとに緊張度スコアを計算：

| 行動 | ウェイト |
|------|---------|
| 陸軍 intimidation × 師団数 | 3.0 |
| 陸軍 offensive × 師団数 | 2.0 |
| 海軍 show_of_force × 艦隊数 | 5.0 |
| 海軍 blockade × 艦隊数 | 8.0 |
| 空軍 recon_flight × 飛行隊数 | 2.0 |
| 空軍 strategic_bombing × 飛行隊数 | 10.0 |

#### 4.4.3. 緊張度レベルと効果

| レベル | スコア | 威嚇される側 | 威嚇する側（民主主義） |
|--------|--------|-------------|---------------------|
| 低 | 0-10 | 影響なし | 影響なし |
| 中 | 10-30 | Rally効果 +1〜+3%/ターン | 軽微 |
| 高 | 30-50 | 国民不安 -1%/ターン | オーディエンスコスト -0.5%/ターン |
| 極高 | 50+ | 恐怖 -3%/ターン + 偶発衝突5% | オーディエンスコスト -1.0%/ターン |

#### 4.4.4. プロンプト注入
`agent/prompts/base.py` の `build_common_context()` に自動注入：
- **威嚇される側**: 緊張度スコア、配備詳細、レベル別効果の説明
- **威嚇する側**: オーディエンスコスト警告（民主主義国家のみ）

### 4.5. Dependencies (依存ライブラリ)
- `geopandas` — GeoJSON読み込み・ポリゴン操作
- `matplotlib` — 地図レンダリング・ダッシュボード出力
- `shapely` — 座標算出・ジオメトリ操作
- `pyproj` — 座標系変換
- データ: `data/geo/ne_10m_admin_0_countries.geojson` (Natural Earth 10m)
