...
$$ \alpha_{mil} = \min\left(\frac{0.20}{4},\ \frac{0.03}{4} + \left(\frac{Military}{\max(1, Y_{t-1})} \times 2.0\right)^2\right) $$
次ターンの軍事力は以下のように算出され、$G_{mil}$ は §2.1 で定義した政策実行力 $\epsilon$ 適用後の軍事予算である。過剰な軍拡は維持費の増大により自壊する。疲弊係数は年率定数を四半期ベースに変換（`/TURNS_PER_YEAR`）して適用される。
...
#### ロジスティック方程式と環境収容力 ($K$)
*   人口増加率はマルサス的（無限）な成長ではなく、国土面積に基づく環境収容力（$K = Area \times 150.0 \div 1,000,000$）に制約される**ロジスティック成長モデル** ($ \frac{dN}{dt} = rN(1 - \frac{N}{K}) $) に従う。**出生率・死亡率は年率で定義されているため、四半期の1ターンに適用する際は `/TURNS_PER_YEAR` で四半期ベースに変換される。** 人口（百万人単位）が $K$（百万人単位）に近づくほど自然増加率はゼロに収束する。
...
*   経済力が $10\%$ 毀損され（$\times 0.9$）、軍事力は前政権から引き継がれ内戦損耗として $10\%$ 減少する（$M = M_{prev} \times 0.9$）。
...
*   新政権の支持率は $50.0\%$ を平均とする正規分布（$\sigma = 15.0$）で決定される（クーデター後の不確実性を反映）。
...
### 経済モデリングのアップデート (2026-05-21)
* 人口動態モデル: 出生率・死亡率（年率定義）を四半期ベースに変換（`/TURNS_PER_YEAR`）し、他のモデルとの時間スケールを統一。以前は年率がそのまま四半期に適用されていたため、人口増加が過大でGDP/C希釈によるマイナス成長を引き起こしていた。
* 軍事維持費モデル（リチャードソン・モデル）: `BASE_MILITARY_MAINTENANCE_ALPHA`/`MAX_MILITARY_FATIGUE_ALPHA`を四半期ベースに変換。以前は年率の維持コストが毎四半期適用されていたため、軍事力の過剰な消耗と経済へのデフレ圧力が過大だった。
* 報道自由度ペナルティ: 報道統制による支持率低下に `country.press_freedom` 乗数を追加。自由な報道に慣れた国民ほど統制に強く反発し、既に統制下にある国民は追加制限への反応が鈍い現実のメカニズムを反映。
* クーデター後の軍事力: `economy * 0.1`（GDPの10%リセット）から `military * 0.9`（前政権引き継ぎ・内戦損耗10%減）に変更。支持率は固定値から正規分布（μ=50, σ=15）に変更。
* SNA詳細ログ: `quarterly_gdp`の除算ゼロ防止を追加（`max(0.1, quarterly_gdp)`）。
* agent/core.py: サブAPIキーフォールバック廃止・リトライ回数4→8に増加、Tool Call無限ループ防止（上限3回・T1無効化）、全APIレスポンスのsystem.log出力を追加。
* db_manager.py: Pythonシャットダウン時のImportError防止のため明示的な`close()`メソッドを追加。
